...
ChatProxy.prototype.connect = function (username) {
  ...
  this.socket.on('connect', function () {
    self.socket.on(Topics.USER_CONNECTED, function (userId) { ///when another user connects to me
      if (userId === self.getUsername()) {  //make sure the other user is not myself
        return;
      }
    ...
    self.socket.on(Topics.USER_DISCONNECTED, function (userId) { //when the other user disconnects
      if (userId === self.getUsername()) {  //make sure it's not myself
        return;
      }
  ...
  this.peer.on('open', function (userId) {             //when the room opens, save the username. 
